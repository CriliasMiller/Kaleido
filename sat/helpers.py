...
def check_if_fsdp2(args):
    """Check if FSDP2 is enabled."""
    return hasattr(args, 'fsdp2') and args.fsdp2

def check_if_fsdp2_full_shard(args):
    """Check if FSDP2 is using full sharding strategy."""
    if not check_if_fsdp2(args):
        return False
    return args.fsdp2_config.get('sharding_strategy', 'full_shard') == 'full_shard'


def debug_fsdp2_param(name, param):
    """Debug FSDP2 parameter including DTensor information."""
    from torch.distributed._tensor import DTensor
    
    if isinstance(param, DTensor):
        local_tensor = param._local_tensor
        print_all(f"FSDP2 DTensor param: {name}, shape: {param.shape}, local_shape: {local_tensor.shape}")
        print_all(f"  local stats - min: {local_tensor.min()}, max: {local_tensor.max()}, "
                 f"mean: {local_tensor.mean()}, std: {local_tensor.std()}")
        
        if param.grad is not None:
            grad_local = param.grad._local_tensor if isinstance(param.grad, DTensor) else param.grad
            print_all(f"  grad local stats - min: {grad_local.min()}, max: {grad_local.max()}, "
                     f"mean: {grad_local.mean()}, std: {grad_local.std()}")
    else:
        print_all(f"FSDP2 regular param: {name}, shape: {param.shape}")
        print_all(f"  stats - min: {param.min()}, max: {param.max()}, "
                 f"mean: {param.mean()}, std: {param.std()}")
        
        if param.grad is not None:
            print_all(f"  grad stats - min: {param.grad.min()}, max: {param.grad.max()}, "
                     f"mean: {param.grad.mean()}, std: {param.grad.std()}")
