...
def add_parallel_config_args(parser):
    """Parallelism configuration arguments."""
    
    group = parser.add_argument_group('parallelism', 'parallelism configurations')
    ...
    group.add_argument('--sequence-parallel-size', type=int, default=1,
                       help='Size of sequence parallelism')
...
def add_model_config_args(parser):
    ...
    group.add_argument('--is-rotary-emb', action='store_true',
                       help='use rotary embedding, common in LLAMA etc.')
...
def add_training_args(parser):
    ...
    group.add_argument('--force-param-sync-interval', type=int, default=0,
                       help='number of iterations between force parameter synchronization acorss tp, sp and dp. 0 represents no force parameter synchronization.')
    group.add_argument('--param-sync-check-interval', type=int, default=0,
                       help='number of iterations between parameter synchronization check acorss tp, sp and dp. 0 represents no parameter synchronization check.')
    group.add_argument('--empty-cache-interval', type=int, default=0,
                       help='number of iterations between empty cache. 0 represents empty cache.')
    group.add_argument('--save-at-start', action='store_true',
                       help='Save checkpoint at step 0.')
...
def initialize_distributed(args):
    ...
    torch.distributed.init_process_group(
        ...
        init_method=init_method, device_id=torch.device(f"cuda:{args.rank}"))
    ...
    if hasattr(args, "num_multi_query_heads"):
        num_multi_query_heads = args.num_multi_query_heads
    mpu.initialize_model_parallel(args.model_parallel_size, sequence_parallel_size, num_multi_query_heads)
