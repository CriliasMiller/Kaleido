...
def save_fsdp2_checkpoint(iteration, model, optimizer, lr_scheduler, args):
    ...
    # Save model state dict (contains DTensors for sharded parameters)
    # FSDP2 state_dict already returns sharded state
    
    # Save optimizer state dict if available
    # if optimizer is not None:
    #     sd['optimizer'] = optimizer.state_dict()

    def fsdp2_state_dict(model):
    ...
    state_dict = fsdp2_state_dict(model) ## dit model
    # torch.save(state_dict, "model_state_dict.pt")
    # Each rank saves its own checkpoint with sharded state
