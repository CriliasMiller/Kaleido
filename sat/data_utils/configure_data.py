...
def make_dataset_full(path, split, args, create_dataset_function, 
        dataset_weights=None, random_mapping=True, is_train_data=False, batch_from_same_dataset=False,restart_on_stop=False, **kwargs):
    ...
    if (is_train_data and args.iterable_dataset) or (not is_train_data and args.iterable_dataset_eval): # cannot indexed
        ...
        ds = AlterDataset(ds, weights=dataset_weights, seed=args.seed, batch_from_same_dataset=batch_from_same_dataset, batch_size=args.batch_size, restart_on_stop=restart_on_stop)
        return ds
...
def make_loaders(args, create_dataset_function, collate_fn=None):
    ...
    if args.train_data is not None:
        train = make_dataset(**data_set_args, args=args, dataset_weights=args.train_data_weights, is_train_data=True,restart_on_stop=args.restart_on_stop)
        if should_split(split):
...
class AlterDataset(IterableDataset):
    def __init__(self, datasets, weights=None, seed=0, batch_from_same_dataset=False, batch_size=1, restart_on_stop=False):
        super().__init__()
        self.seed = seed
        ...
        while True:
            ...
            try:
                ...
                if self.restart_on_stop:
                    # reset this dataset to a new epoch
                    iterators[index] = iter(self.datasets[index])
                    from sat.helpers import print_rank0
                    print_rank0(f'AlterDataset: restart a dataset, {len(iterators)} left.')
                    continue
                else:
                    # remove this dataset
                    del iterators[index]
