...
from ..util import get_context_parallel_group, get_context_parallel_rank, get_context_parallel_world_size, \
...
class SpatialNorm3D(nn.Module):
    ...
    def forward(self, f, zq, clear_fake_cp_cache=True, fake_cp_rank0=True):
        if f.shape[2] > 1 and get_context_parallel_rank() == 0 and fake_cp_rank0:
            ...
            interpolated_splits = [torch.nn.functional.interpolate(split, size=f_rest_size, mode="nearest") \
            ...
            interpolated_splits = [torch.nn.functional.interpolate(split, size=f_size, mode="nearest") \
    ...
    def forward(self, x, fake_cp_rank0=True):
        if self.compress_time and x.shape[2] > 1:
            if get_context_parallel_rank() == 0 and fake_cp_rank0:
                ...
                interpolated_splits = [torch.nn.functional.interpolate(split, scale_factor=2.0, mode="nearest") for split in splits]
    ...
    def forward(self, x, fake_cp_rank0=True):
        if self.compress_time and x.shape[2] > 1:
            ...
            if get_context_parallel_rank() == 0 and fake_cp_rank0:
                ...
                if x_rest.shape[-1] > 0:
                    splits = torch.split(x_rest, 32, dim=1)
                    interpolated_splits = [torch.nn.functional.avg_pool1d(split, kernel_size=2, stride=2) for split in splits]
                    x_rest = torch.cat(interpolated_splits, dim=1)
...
class ContextParallelEncoder3D(nn.Module):
    ...
    def forward(self, x, clear_fake_cp_cache=True, fake_cp_rank0=True, **kwargs):
...
class ContextParallelDecoder3D(nn.Module):
    def __init__(self, *, ch, out_ch, ch_mult=(1,2,4,8), num_res_blocks,
                 attn_resolutions, dropout=0.0, resamp_with_conv=True, in_channels, resolution, z_channels, give_pre_end=False, zq_ch=None, add_conv=False, pad_mode='first', temporal_compress_times=4, gather_norm=False, reverse=False, **ignorekwargs):
        super().__init__()
        self.ch = ch
        self.temb_ch = 0
        self.num_resolutions = len(ch_mult)
    ...
    def forward(self, z, clear_fake_cp_cache=True, fake_cp_rank0=True, **kwargs):
        ...
        for i_level in reversed(range(self.num_resolutions)):
            for i_block in range(self.num_res_blocks+1):
                h = self.up[i_level].block[i_block](h, temb, zq, clear_fake_cp_cache=clear_fake_cp_cache, fake_cp_rank0=fake_cp_rank0)
