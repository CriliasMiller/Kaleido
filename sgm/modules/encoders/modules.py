...
class GeneralConditioner(nn.Module):
    ...
    def __init__(self, emb_models: Union[List, ListConfig], cor_embs=[], cor_p=[]):
        ...
        for n, embconfig in enumerate(emb_models):
            ...
            embedder.legacy_ucg_val = embconfig.get("legacy_ucg_val", None)
    ...
    def get_single_embedding(self, embedder, batch, output, cond_or_not: Optional[np.ndarray] = None, force_zero_embeddings: Optional[List] = None):
        ...
        for emb in emb_out:
            ...
            if out_key in output:
                ...
                output[out_key] = emb
        return output, batch[embedder.input_key]
    ...
    def forward(
        ...
        if len(self.cor_embs) > 0:
            ...
            for emb_idx in self.cor_embs:
                cond_or_not = rand_idx % 2 # 1表示用uncond, 0表示用cond
                rand_idx //= 2
        ...
        for i, embedder in enumerate(self.embedders):
            if i in self.cor_embs:
                continue
            output, cond_text = self.get_single_embedding(embedder, batch, output=output, force_zero_embeddings=force_zero_embeddings)
        return output, cond_text
    ...
    def get_unconditional_conditioning(
        ...
        c, _ = self(batch_c)
        uc, _ = self(batch_c if batch_uc is None else batch_uc, force_uc_zero_embeddings)
...
class FrozenMistralEmbedder(AbstractEmbModel):
    """Uses the Mistral for text"""

    def __init__(
        self, version="mistralai/Mistral-7B-v0.1", device="cuda", max_length=77, freeze=True, use_penultimate=True, use_lora=False
    ): 
        super().__init__()
        self.tokenizer = AutoTokenizer.from_pretrained(version, local_files_only=True)
        self.tokenizer.pad_token = self.tokenizer.eos_token
        self.model = AutoModel.from_pretrained(version, local_files_only=True)
        self.device = device
        ...
        if use_lora:
            lora_config = LoraConfig(inference_mode=False, r=32, lora_alpha=32, lora_dropout=0.1)
...
class FrozenT5Embedder(AbstractEmbModel):
    ...
    def __init__(
        self, version="google/t5-v1_1-xxl", device="cuda", max_length=77, freeze=True,
            cache_dir=None, forward_with_mask=False, set_padding_zero=False, varlen_text=False, uncond_text_length=1,
    ):  # others are google/t5-v1_1-xl and google/t5-v1_1-xxl
        super().__init__()
        self.tokenizer = T5Tokenizer.from_pretrained(version, local_files_only=True, cache_dir=cache_dir)
        self.transformer = T5EncoderModel.from_pretrained(version, local_files_only=True, cache_dir=cache_dir)
        self.device = device
        self.max_length = max_length
        self.forward_with_mask = forward_with_mask
        self.set_padding_zero = set_padding_zero
        self.varlen_text = varlen_text
        self.cond_length_multiple = get_model_parallel_world_size() * get_sequence_parallel_world_size()
        self.uncond_text_length = uncond_text_length
        if freeze:
            self.freeze()
    ...
    def forward(self, text):
        ...
        with torch.autocast("cuda", enabled=False):
            outputs = self.transformer(input_ids=tokens, attention_mask=attention_mask if self.forward_with_mask else None)
        z = outputs.last_hidden_state
        if self.set_padding_zero:
            z = z * attention_mask[:, :, None]
        if self.varlen_text:
            assert z.shape[0] == 1
            num_ones = attention_mask[0].sum().item()
            padding = 0
            if num_ones > 1:
                if num_ones % self.cond_length_multiple != 0:
                    padding = (self.cond_length_multiple - (num_ones % self.cond_length_multiple)) % self.cond_length_multiple
            else:
                padding = self.uncond_text_length - num_ones
            z = z[:, :num_ones + padding]
        return z

    def encode(self, text):
        return self(text)

# class FrozenUmT5Embedder(AbstractEmbModel):
#     """Uses the T5 transformer encoder for text"""
#
#     def __init__(
#         self, version="google/t5-v1_1-xxl", device="cuda", max_length=77, freeze=True,
#             tokenizer_path=None, ckpt_path=None, forward_with_mask=False, set_padding_zero=False, varlen_text=False, uncond_text_length=1,
#     ):  # others are google/t5-v1_1-xl and google/t5-v1_1-xxl
#         super().__init__()
#         self.tokenizer = AutoTokenizer.from_pretrained(tokenizer_path, local_files_only=True)
#         self.transformer = UMT5EncoderModel(UMT5Config.from_pretrained(tokenizer_path, local_files_only=True))
#         self.device = device
#         self.max_length = max_length
#         self.forward_with_mask = forward_with_mask
#         self.set_padding_zero = set_padding_zero
...
class FrozenSiglipEmbedder(AbstractEmbModel):
    ...
    def __init__(
            ...
            freeze=True,
            cache_dir=None,
    ):  # others are google/t5-v1_1-xl and google/t5-v1··_1-xxl
        super().__init__()
        ...
        self.model = SiglipTextModel.from_pretrained("google/siglip-so400m-patch14-384", cache_dir=cache_dir,
                                                     local_files_only=True, **model_config)
        self.tokenizer = AutoTokenizer.from_pretrained("google/siglip-so400m-patch14-384", cache_dir=cache_dir,
    ...
    def forward(self, input: Union[str, torch.Tensor], input_mask: Union[str, torch.Tensor] = None):
        if isinstance(input, str) or isinstance(input, list):
            ...
            # skip tokenizer if input is already tokenized
...
class HiddenSpaceEmbedder(AbstractEmbModel):
    """Uses the CLIP transformer encoder for text (from huggingface)"""
    ...
    def __init__(
        ...
        super().__init__()
        self.model = instantiate_from_config(autoencoder_config)
        self.freeze()
    ...
    def forward(self, image):
        with torch.autocast("cuda", enabled=False):
            ...
            outputs = self.model.encode(image)
            #[b,c,h,w]
            outputs = F.upsample_nearest(outputs, scale_factor=8)
            return outputs
