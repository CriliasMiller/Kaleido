...
from ...util import append_zero, append_one
...
class RFDiscretization(Discretization):
    def __init__(
        self,
        num_timesteps=1000,
        reverse=False,
        ...
        if reverse:
            self.sigmas = np.linspace(1, 0, num_timesteps+1)[1:]
        else:
            self.sigmas = np.linspace(0, 1, num_timesteps+1)[1:]
    ...
    def get_sigmas(self, n, device="cpu", return_idx=False):
        ...
        to_torch = partial(torch.tensor, dtype=torch.float32, device=device)
        sigmas = to_torch(sigmas)

        if return_idx:
            return torch.flip(sigmas, (0,)), timesteps
        else:
    ...
    def __call__(self, n, do_append_zero=True, device="cpu", flip=False, return_idx=False):
        if return_idx:
            ...
            sigmas = self.get_sigmas(n, device=device, return_idx=return_idx)
        if self.reverse:
            sigmas = append_one(sigmas) if do_append_zero else sigmas
        else:
            sigmas = append_zero(sigmas) if do_append_zero else sigmas
        if return_idx:
            return sigmas if not flip else torch.flip(sigmas, (0,)), idx
