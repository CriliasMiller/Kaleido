...
from ...modules.diffusionmodules.loss import get_lin_function, time_shift
...
class VideoDDIMSampler(BaseDiffusionSampler):
    ...
    def denoise(self, x, denoiser, alpha_cumprod_sqrt, cond, uc, timestep=None, idx=None, scale=None, scale_emb=None, ofs=None):
        ...
        if isinstance(scale, torch.Tensor) == False and scale == 1:
            ...
            # if scale_emb is not None:
            #     additional_model_inputs['scale_emb'] = torch.cat([scale_emb]*2, dim=0)
...
def make_hunyuan_timesteps(t_start: int, num_flow_steps: int, verbose=True, shift_scale = 7, mode='normal'):
    '''Select `num_meta_steps` timesteps for meta sampling from t_start down to 0'''
    steps_out = np.linspace(t_start, 1., num_flow_steps + 1, endpoint=True)
    steps_out = steps_out / (shift_scale + steps_out - shift_scale * steps_out)
    steps_out = torch.tensor(steps_out, dtype=torch.float32)

    if mode == 'meta':
        steps_out = steps_out
    elif mode == 'normal':
        steps_out = 1 - steps_out
    else:
        raise ValueError(f"Unknown mode {mode}.")
    
    if verbose:
        print(f'Selected timesteps for flow sampler: {steps_out}')
    return steps_out

class RFSampler(BaseDiffusionSampler):

    def __init__(self, schedule_shift=False, hunyuan_schedule=False, shift_scale=7, mode='normal', adapter_emb=False, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.schedule_shift = schedule_shift
        self.hunyuan_schedule = hunyuan_schedule
        self.shift_scale = shift_scale
        self.mode = mode
        self.adapter_emb = adapter_emb

    def prepare_sampling_loop(self, x, cond, uc=None, num_steps=None):
        sigmas, timesteps = self.discretization(
            self.num_steps if num_steps is None else num_steps, device=self.device, return_idx=True
        )

        if self.schedule_shift:
            image_seq_len = x.shape[-1] * x.shape[-2] # 注意这里要求x必须是16倍压缩
            mu = get_lin_function(y1=0.5, y2=1.15)(image_seq_len)
            sigmas = time_shift(mu, sigmas, mode=self.mode)
        if self.hunyuan_schedule:
            sigmas = make_hunyuan_timesteps(0, self.num_steps, mode=self.mode, shift_scale=self.shift_scale)
        ...
        return x, s_in, sigmas, num_sigmas, cond, uc, timesteps

    def denoise(self, x, denoiser, sigma, cond, uc, scale=None, scale_emb=None, fps=None, output_adapter_states=None):
        additional_model_inputs = {}
        if fps is not None:
            additional_model_inputs['fps'] = fps
        if not self.adapter_emb: 
            denoised = denoiser(*self.guider.prepare_inputs(x, sigma, cond, uc), **additional_model_inputs).to(torch.float32)
            denoised = self.guider(denoised, sigma)
            return denoised
        else:
            additional_model_inputs['condition_hidden_states'] = output_adapter_states
            additional_model_inputs["return_cache"] = True if output_adapter_states is None else False
            if output_adapter_states is None:
                denoised, output_adapter_states = denoiser(*self.guider.prepare_inputs(x, sigma, cond, uc), **additional_model_inputs)
                denoised = self.guider(denoised.to(torch.float32), sigma)
                return denoised, output_adapter_states
            else:
                denoised = denoiser(*self.guider.prepare_inputs(x, sigma, cond, uc), **additional_model_inputs)
                denoised = self.guider(denoised.to(torch.float32), sigma)
                return denoised
    
    def sampler_step(self, sigma, next_sigma, denoiser, x, cond, uc=None, scale=None, scale_emb=None, fps=None, output_adapter_states=None):
        if not self.adapter_emb: 
            output = self.denoise(x, denoiser, sigma, cond, uc, scale=scale, scale_emb=scale_emb, fps=fps).to(torch.float32)
            x = x + append_dims(next_sigma - sigma, x.ndim) * output
            return x
        else:
            if output_adapter_states is None:
                output, output_adapter_states = self.denoise(x, denoiser, sigma, cond, uc, scale=scale, scale_emb=scale_emb, fps=fps, output_adapter_states=output_adapter_states)
                output = output.to(torch.float32)
                x = x + append_dims(next_sigma - sigma, x.ndim) * output
                return x, output_adapter_states
            else:
                output = self.denoise(x, denoiser, sigma, cond, uc, scale=scale, scale_emb=scale_emb, fps=fps, output_adapter_states=output_adapter_states)
                output = output.to(torch.float32)
                x = x + append_dims(next_sigma - sigma, x.ndim) * output
                return x

    def __call__(self, denoiser, x, cond, uc=None, num_steps=None, scale=None, scale_emb=None, ofs=None, fps=None):
        ...
        if not self.adapter_emb:
            ...
            for i in self.get_sigma_gen(num_sigmas):
                if sample_times == 0:
                    x, output_adapter_states = self.sampler_step(
                        s_in * sigmas[i],
                        s_in * sigmas[i + 1],
                        denoiser,
                        x,
                    ...
                    x = self.sampler_step(
                        ...
                        scale_emb=scale_emb,
                        fps=fps,
                        output_adapter_states=output_adapter_states,
                    )
                sample_times += 1
        return x