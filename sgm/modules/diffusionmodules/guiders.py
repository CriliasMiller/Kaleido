...
class VanillaCFG:
    ...
    def prepare_inputs(self, x, s, c, uc):
        ...
        for k in c:
            if k in ["vector", "crossattn", "concat", 'concat_subjects'] and isinstance(c[k], torch.Tensor):
                if uc[k].shape[1] != c[k].shape[1]:
                    uc[k] = torch.cat([uc[k], uc[k][:,-1:].repeat(1, abs(c[k].shape[1]-uc[k].shape[1]), 1)], dim=1)
...
class DynamicCFG(VanillaCFG):
    def __init__(self, scale, exp, num_steps, dyn_thresh_config=None):
        ...
        scale_schedule = lambda scale, sigma, step_index: \
            1 + scale * (1 - math.cos(math.pi* (step_index/num_steps)**exp)) / 2
        self.scale_schedule = partial(scale_schedule, scale)
        self.dyn_thresh = instantiate_from_config(
            default(
                dyn_thresh_config,
...
class NewLinearPredictionGuider(Guider):
    def __init__(
        self,
        max_scale: float,
        num_frames: int,
    ...
    def __call__(self, x: torch.Tensor, sigma: torch.Tensor, **kwargs) -> torch.Tensor:
        ...
        return x_u + scale * (x_c - x_u)

    def prepare_inputs(
        self, x: torch.Tensor, s: torch.Tensor, c: dict, uc: dict, **kwargs
